...
def daftar_pasien_baru(
    ...
    if (
        data.nik
        and db.query(models.Pasien)
        .filter(models.Pasien.nik == data.nik)
        .first()
    ):
        raise HTTPException(
            status_code=400,
...
def get_pasien(
    ...
    riwayat = (
        ...
        .order_by(
            models.RekamMedis.waktu_pemeriksaan.desc()
        )
        .all()
    )
...
def cari_pasien(
    ...
    if not hasil:
        return {
            "msg": f"Tidak ada pasien dengan nama yang mengandung '{nama}'.",
            "data": [],
        }
...
def get_rekam_medis(
    ...
    return {
        ...
        "tanggal_lahir": (
            str(rm.pasien.tanggal_lahir)
            if rm.pasien and rm.pasien.tanggal_lahir
            else None
        ),
